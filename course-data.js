const QuranCourseData = {
    chapters: [
        {
            id: 1,
            title: "<span class='font-amiri fw-bold text-primary'>أَنْوَاع الْكَلِمَة</span> (Word Types)",
            description: "Learn how every word in the Quran is either a Fil (Verb), an Ism (Name), or a Huroof (Particle).",
            url: "chapter1.html",
            color: "primary"
        },
        {
            id: 2,
            title: "<span class='font-amiri fw-bold text-primary'>جِذْر</span> (The Root)",
            description: "Understand the fundamental DNA of Arabic grammar through four core transformations (with two advanced forms).",
            url: "chapter2.html",
            color: "success"
        },
        {
            id: 3,
            title: "<span class='font-amiri fw-bold text-primary'>الضَّمِير</span> (Pronouns)",
            description: "A consolidated master reference table combining pronouns and both verb tenses for quick lookups.",
            url: "chapter3.html",
            color: "primary"
        },
        {
            id: 4,
            title: "<span class='font-amiri fw-bold text-primary'>فِعْل - مُذَكَّر</span> (Verbs - Masculine)",
            description: "Master the masculine pronouns attached and detached, along with their verb tense endings.",
            url: "chapter4.html",
            color: "primary"
        },
        {
            id: 5,
            title: "<span class='font-amiri fw-bold text-primary'>فِعْل - مُؤَنَّث</span> (Verbs - Feminine)",
            description: "Master the feminine pronouns attached and detached, along with their verb tense endings.",
            url: "chapter5.html",
            color: "primary"
        },
        {
            id: 6,
            title: "<span class='font-amiri fw-bold text-primary'>الْجَمْع</span> (Plurals)",
            description: "Learn how to identify plurals in the Quran, from the regular 'Sound' endings to the unique 'Broken' patterns.",
            url: "chapter6.html",
            color: "success"
        },
        {
            id: 7,
            title: "<span class='font-amiri fw-bold text-primary'>حُرُوف الْجَرّ</span> (Prepositions)",
            description: "Master the 'Huroof al-Jarr'—the power particles like Fi, Bi, and Min that change word endings and connect meanings across the Quran.",
            url: "chapter7.html",
            color: "danger"
        },
        {
            id: 8,
            title: "<span class='font-amiri fw-bold text-primary'>أَوْزَان</span> (Verb Forms)",
            description: "See how roots upgrade their meaning across all 10 standard verb forms (1 to 10) in the Quran.",
            url: "chapter8.html",
            color: "primary"
        },
        {
            id: 9,
            title: "<span class='font-amiri fw-bold text-primary'>مَرْفُوع مَنْصُوب مَجْرُور</span> (Three states)",
            description: "Hear the last syllable: marfu‘ (often u / ḍamma), mansūb (often a / fatḥa), majrūr (often i / kasra)—plus simple pause (waqf) sights before full chapters on verbs and jar.",
            url: "chapter9.html",
            color: "primary"
        }
    ]
};

